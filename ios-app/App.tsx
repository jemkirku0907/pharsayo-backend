...
const navItems: { id: Screen; label: string; icon: string }[] = [
  { id: 'home', label: 'Ngayon', icon: '⌂' },
  { id: 'medicines', label: 'Gamot', icon: '✚' },
  { id: 'scan', label: 'I-scan', icon: '⌗' },
  { id: 'reminders', label: 'Paalaala', icon: '◷' },
  { id: 'support', label: 'BHU', icon: '♡' },
];
    ...
    <SafeAreaView style={styles.loginSafe}>
      ...
      <KeyboardAvoidingView style={styles.loginKeyboard} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.loginScroll} keyboardShouldPersistTaps="handled">
          <View style={styles.brandRow}><View style={styles.logo}><Text style={styles.logoGlyph}>♡</Text></View><Text style={styles.brand}>PharSayo</Text></View>
          ...
          <View style={styles.loginCard}>
            ...
            <TextInput value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" autoComplete="email" placeholder="hal. maria@email.com" placeholderTextColor="#93A29D" style={styles.input} />
            <Text style={styles.label}>Password</Text>
            <View style={styles.passwordWrap}><TextInput value={password} onChangeText={setPassword} secureTextEntry={secure} autoComplete="current-password" placeholder="Ilagay ang password" placeholderTextColor="#93A29D" style={styles.passwordInput} /><Pressable onPress={() => setSecure(!secure)}><Text style={styles.showPassword}>{secure ? 'Ipakita' : 'Itago'}</Text></Pressable></View>
            <Pressable onPress={signIn} disabled={loading} style={({ pressed }) => [styles.primaryButton, pressed && styles.pressed, loading && styles.disabled]}>{loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryButtonText}>Mag-login  →</Text>}</Pressable>
...
function HomeScreen({ medicines, setMedicines, onNavigate }: { medicines: Medicine[]; setMedicines: React.Dispatch<React.SetStateAction<Medicine[]>>; onNavigate: (screen: Screen) => void }) {
  ...
  return <ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.quickGrid}>{[
      ['scan', '⌗', 'Kilalanin ang gamot'], ['medicines', '✚', 'Mga gamot ko'], ['reminders', '◷', 'Gumawa ng reminder'], ['support', '♡', 'BHU support'],
    ].map(([id, icon, label]) => <Pressable key={id} onPress={() => onNavigate(id as Screen)} style={styles.quickCard}><View style={styles.quickIcon}><Text style={styles.quickIconText}>{icon}</Text></View><Text style={styles.quickLabel}>{label}</Text><Text style={styles.quickArrow}>›</Text></Pressable>)}</View>
  </ScrollView>;
}
